...
在查询后的调用链列表中，单击待查看的调用链前的![](figures/zh-cn_image_0000001087791142.png)，查看该调用链基本信息，如下图所示。

![](figures/zh-cn_image_0000001087789054.png)
...
-   下半部分为调用链详细的方法堆栈，每一行代表一个方法调用，展示了调用链内部方法的详细调用关系。默认只展示javaagent支持的组件方法，如需展示业务方法，可以在JavaMethod配置中配置需要拦截的业务方法。
    ...
    ![](figures/zh-cn_image_0000001083581254.png)
